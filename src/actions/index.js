...
export function selectedMovie(movieID) {
	...
	const url = `${ROOT_URL}/Schedule/?eventID=${movieID}`;
	const selected = axios.get(url).then((xmlData) => {
		...
		let movieUnmapped = jsonObj.Schedule.Shows.Show
		return movieUnmapped
	});

	if (Array.isArray(selected)) {
		let res = selected.filter((ele, ind) => ind === selected.findIndex(elem => elem.EventID === ele.EventID))
		return {
			type: SELECTED_MOVIE,
			payload: res
		};
